...
struct CommonFeaturesTest : public testing::TestWithParam<DataSample> {
  // MerkleDAG service
  std::shared_ptr<Service> merkledag_service_{nullptr};
  ...
  void SetUp() override {
    ...
    merkledag_service_ = std::make_shared<ServiceImpl>(blockservice);
    data = GetParam();
   ...
   * @param leave - root node
   * @return operation result
   */
  static std::string getGraphStructure(const Leave &leave) {
    std::string content{"{["};
    std::string data{leave.content().begin(), leave.content().end()};
    content.append(data);
    content.push_back(']');
    std::vector<std::string_view> leaves = leave.getSubLeaveNames();
    if (!leaves.empty()) {
      content.append("->");
      for (const auto &leave_id : leaves) {
        EXPECT_OUTCOME_TRUE(sub_leave, leave.subLeave(leave_id))
        content.append(getGraphStructure(sub_leave));
        content.push_back(',');
      }
      content.pop_back();
    }
...
TEST_P(CommonFeaturesTest, FetchGraphSuccess) {
  ...
  EXPECT_OUTCOME_TRUE(root_leave, merkledag_service_->fetchGraph(root_cid))
  std::string fetched_structure = getGraphStructure(*root_leave);
