...
using fc::primitives::sector_file::kOverheadDenominator;
using fc::primitives::sector_file::kOverheadSeal;
using fc::primitives::sector_file::SectorFile;
...
class SectorFileTest : public test::BaseFS_Test {
 ...
 protected:
  ...
  RegisteredProof border_seal_proof_type = RegisteredProof::StackedDRG8MiBSeal;
...
TEST_F(SectorFileTest, CreatedFileEmpty) {
  EXPECT_OUTCOME_TRUE(
      sector_size, fc::primitives::sector::getSectorSize(min_seal_proof_type));
  SectorId sector{
      .miner = 1,
      .sector = 1,
  };
...
TEST_F(SectorFileTest, WritePieceWtihBlank) {
  ...
  EXPECT_OUTCOME_TRUE_1(
      file->write(PieceData(resourcePath("payload.txt")), offset, piece_size));
...
TEST_F(SectorFileTest, ReadPiece) {
  ...
  EXPECT_OUTCOME_TRUE(read_data, fc::common::readFile(read_file_path.string()));
...
TEST_F(SectorFileTest, SealSpaceUse) {
  ...
  EXPECT_OUTCOME_TRUE(seal_size, sealSpaceUse(file_type, min_seal_proof_type));
