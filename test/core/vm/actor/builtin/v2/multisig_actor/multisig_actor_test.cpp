...
namespace fc::vm::actor::builtin::v2::multisig {
  ...
  TEST_F(MultisigActorTest, ConstrucCorrect) {
    ...
    EXPECT_OUTCOME_TRUE_1(Construct::call(runtime, {signers, threshold, start_epoch}));
  }
  ...
  TEST_F(MultisigActorTest, CancelWrongTxId) {
    ...
    EXPECT_OUTCOME_ERROR(VMExitCode::kErrNotFound, Cancel::call(runtime, {}));
  }
  ...
  /**
   * @given Runtime and multisig actor
   * @when lockBalance is called with epoch > network version 2 and negative amount
   * @then error returned
   */
