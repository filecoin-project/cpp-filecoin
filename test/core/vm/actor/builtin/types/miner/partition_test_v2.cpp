...
namespace fc::vm::actor::builtin::v2::miner {
  ...
  struct PartitionTestV2 : testing::Test {
    void SetUp() override {
  ...
  TEST_F(PartitionTestV2, FailsToAddFaultsForMissingSectors) {
    ...
    const auto result = partition.declareFaults(
        runtime, sectors_arr, fault_set, 7, ssize, quant);
  ...
  TEST_F(PartitionTestV2, AddsRecoveries) {
    ...
    EXPECT_OUTCOME_TRUE_1(partition.declareFaults(
        runtime, sectors_arr, fault_set, 7, ssize, quant));
  ...
  TEST_F(PartitionTestV2, RemoveRecoveries) {
    ...
    EXPECT_OUTCOME_TRUE(
        result1,
        partition.declareFaults(runtime, sectors_arr, {}, 7, ssize, quant));
    ...
    EXPECT_OUTCOME_TRUE(
        result2,
        partition.declareFaults(runtime, sectors_arr, {5}, 10, ssize, quant));
  ...
  TEST_F(PartitionTestV2, ReschedulesExpirations) {
    setup();
    const auto unproven_sector = Utils::testSector(13, 7, 55, 65, 1006);
  ...
  TEST_F(PartitionTestV2, PopExpiringSectorsErrorsIfARecoveryExists) {
    ...
    EXPECT_OUTCOME_TRUE_1(
        partition.declareFaults(runtime, sectors_arr, {5}, 2, ssize, quant));
    EXPECT_OUTCOME_TRUE_1(
  ...
  TEST_F(PartitionTestV2, RecordsMissingPost) {
    ...
    auto faulty_sectors = Utils::slice(sectors, 0, 3);
    ...
    EXPECT_EQ(power_delta,
              powerForSectors(ssize, Utils::slice(sectors, 0, 3)).negative());
