...
namespace fc::vm::actor::builtin::v2::miner {
  ...
  struct ExpirationQueueTestV2 : testing::Test {
    void SetUp() override {
      ...
      sectors = {testSector(ActorVersion::kVersion2, 2, 1, 50, 60, 1000),
                 testSector(ActorVersion::kVersion2, 3, 2, 51, 61, 1001),
                 testSector(ActorVersion::kVersion2, 7, 3, 52, 62, 1002),
                 testSector(ActorVersion::kVersion2, 8, 4, 53, 63, 1003),
                 testSector(ActorVersion::kVersion2, 11, 5, 54, 64, 1004),
                 testSector(ActorVersion::kVersion2, 13, 6, 55, 65, 1005)};
    }
  ...
  TEST_F(ExpirationQueueTestV2, AddedSectorsCanBePoppedOffQueue) {
  ...
  TEST_F(ExpirationQueueTestV2, QuantizesAddedSectorsByExpiration) {
  ...
  TEST_F(ExpirationQueueTestV2, ReschedulesSectorsToExpireLater) {
  ...
  TEST_F(ExpirationQueueTestV2, ReschedulesSectorsAsFaults) {
  ...
  TEST_F(ExpirationQueueTestV2, ReschedulesAllSectorsAsFaults) {
    eq.quant = QuantSpec(4, 1);
    EXPECT_OUTCOME_TRUE_1(eq.addActiveSectors(sectors, ssize));
  ...
  TEST_F(ExpirationQueueTestV2, RescheduleExpirationsThenRescheduleAsFault) {
    eq.quant = QuantSpec(4, 1);
    EXPECT_OUTCOME_TRUE_1(eq.addActiveSectors(sectors, ssize));
    ...
    const RleBitset expected_on_time_sectors1{1, 2, 3};
    EXPECT_EQ(es1.on_time_sectors, expected_on_time_sectors1);
    EXPECT_EQ(es1.active_power, powerForSectors(ssize, {sectors[0]}));
    EXPECT_EQ(es1.faulty_power, expected_power);

    EXPECT_OUTCOME_TRUE(es2, eq.popUntil(20));
    const RleBitset expected_on_time_sectors2{4, 5, 6};
    EXPECT_EQ(es2.on_time_sectors, expected_on_time_sectors2);
    EXPECT_EQ(es2.active_power,
              powerForSectors(ssize, {sectors[3], sectors[4], sectors[5]}));
    EXPECT_EQ(es2.faulty_power, PowerPair());
  }

  TEST_F(ExpirationQueueTestV2, RescheduleRecoverRestoresAllSectorStats) {
    eq.quant = QuantSpec(4, 1);
    EXPECT_OUTCOME_TRUE_1(eq.addActiveSectors(sectors, ssize));
  ...
  TEST_F(ExpirationQueueTestV2, ReplacesSectorsWithNewSectors) {
    eq.quant = QuantSpec(4, 1);
  ...
  TEST_F(ExpirationQueueTestV2, RemovesSectors) {
    eq.quant = QuantSpec(4, 1);
    EXPECT_OUTCOME_TRUE_1(eq.addActiveSectors(sectors, ssize));
  ...
  TEST_F(ExpirationQueueTestV2, AddingNoSectorsLeavesTheQueueEmpty) {
    eq.quant = QuantSpec(4, 1);
  ...
  TEST_F(ExpirationQueueTestV2, ReschedulingNoExpirationsLeavesTheQueueEmpty) {
    eq.quant = QuantSpec(4, 1);
