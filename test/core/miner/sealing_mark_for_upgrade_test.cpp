...
namespace fc::mining {
   ...
   */
  TEST_F(SealingMarkForUpgradeTest, MarkForUpgradeNotProvingState) {
   ...
   */
  TEST_F(SealingMarkForUpgradeTest, MarkForUpgradeSeveralPieces) {
    EXPECT_CALL(*counter_, next()).WillOnce(testing::Return(sector));
    EXPECT_CALL(*manager_,
                doAddPieceSync(
                    sector_ref, IsEmpty(), piece_size, _, kDealSectorPriority))
   ...
   */
  TEST_F(SealingMarkForUpgradeTest, MarkForUpgradeWithDeal) {
    EXPECT_CALL(*counter_, next()).WillOnce(testing::Return(sector));
    EXPECT_CALL(*manager_,
                doAddPieceSync(
                    sector_ref, IsEmpty(), piece_size, _, kDealSectorPriority))
   ...
   * @then success
   */
  TEST_F(SealingMarkForUpgradeTest, MarkForUpgrade) {
   ...
   */
  TEST_F(SealingMarkForUpgradeTest, MarkForUpgradeAlreadyMarked) {
    EXPECT_OUTCOME_TRUE_1(sealing_->markForUpgrade(update_sector_id_));
    EXPECT_EQ(sealing_->isMarkedForUpgrade(update_sector_id_), true);
