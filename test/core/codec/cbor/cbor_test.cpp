...
namespace fc::codec::cbor {
  ...
  /**
   * @given Map container with duplicate items
   * @when Encode
   * @then Exception is thrown
   */
  TEST(CborEncoder, MapDuplicate) {
    CborEncodeStream s;
    auto map = CborEncodeStream::map();
    map["aa"] << 1;
    map["aa"] << 2;
  ...
  TEST(CborEncoder, MapRespectsOrderDuplicateKey) {
    ...
    map["c"] << 3;
    s << map;
    EXPECT_EQ(s.data(), "A1616303"_unhex) << "Got " << hex_upper(s.data());
  }
