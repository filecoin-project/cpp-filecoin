...
/**
 * @class Sha256Test is used to test sha256 functionality
 */
...
/**
 * @given set of pairs value -> digest
 * @when sha256 is applied to value
 * @then obtained result is equal to digest
 */
TEST_F(Sha256Test, Valid) {
  for (const auto &[value, digest] : test_vectors) {
    ASSERT_EQ(sha256(value).toHex(), digest);
  }
}
