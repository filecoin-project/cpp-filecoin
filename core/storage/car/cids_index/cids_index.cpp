...
namespace fc::storage::cids_index {
  ...
  outcome::result<void> merge(std::ostream &out,
    ...
    if (!common::write1(out, kHeaderV0)) {
      return write_error;
    }
    while (!ranges.empty()) {
      std::pop_heap(ranges.begin(), ranges.end(), cmp);
      auto &range{ranges.back()};
      if (!common::write1(out, range.front())) {
        return write_error;
      }
      range.pop();
      if (range.empty()) {
    ...
    if (!common::write1(out, kTrailerV0)) {
      return write_error;
    }
    out.flush();
  ...
  outcome::result<size_t> readCar(std::istream &car_file,
    ...
    if (!common::write1(rows_file, kHeaderV0)) {
      return write_error;
    }
    ...
    if (!common::write1(rows_file, kTrailerV0)) {
      return write_error;
    }
    rows_file.flush();
    return total;
  ...
  outcome::result<boost::optional<Row>> SparseIndex::find(
    ...
    for (auto i{i_begin}; i <= i_end; ++i) {
      Row row;
      if (!common::read1(index_file, row)) {
