...
namespace fc::storage::ipfs::merkledag {
  ...
  const CID &NodeImpl::getCID() const {
    if (!cid_) {
      ...
      CID id{ Version::V0, MulticodecType::DAG_PB, std::move(multi_hash.value())};
      cid_ = std::move(id);
    }
    return cid_.value();
  }
  ...
  outcome::result<std::shared_ptr<Node>> NodeImpl::createFromRawBytes(
    ...
    for (size_t i = 0; i < decoder.getLinksCount(); ++i) {
      ...
      LinkImpl link{std::move(link_cid),
                    decoder.getLinkName(i),
                    decoder.getLinkSize(i)};
      node->addLink(link);
    }
    return node;
  }
