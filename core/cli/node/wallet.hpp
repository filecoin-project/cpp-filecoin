...
namespace fc::cli::_node {
  ...
  struct walletList {
    struct Args {
      ...
      CLI_RUN() {
        ...
        Address def = cliTry(api._->WalletDefaultAddress(),
  ...
  /*
      struct walletBalance {
        struct Args {
          ...
          CLI_OPTS() {
            ...
            option("address,a", po::value(&addressString)->required(),
     "Address"); return opts;
          }
        };
        ...
        CLI_RUN() {
          ...
          CLI_TRY_TEXT(balance, api._->WalletBalance(address), "Can`t get
      balance");

          if (balance == 0) {
            std::cout << balance
                      << " (warning: may display 0 if chain sync in
     ...
     "Address"); return opts;
      ...
      message"); return opts;
        ...
        CLI_RUN() {
          ...
          CLI_TRY_TEXT(
              decode_message, unhex(args.hexMessage), "Can`t decode hex
     message");
     ...
     verify");
      ...
      address") CLI_TRY_TEXT( decode_message, unhex(args.hex_message), "Can`t
      decode hex message"); CLI_TRY_TEXT( signing_bytes, unhex(args.signature),
      ...
      available"); option("wallet,w", po::value(&wallet)->required(), "Specify
      address to withdraw funds to, otherwise it will use " "the default wallet
      address"); option("address,a", po::value(&address)->required(), "Market
      address to withdraw from (account or miner actor " "address, defaults to
     ...
     invalid");
          ...
          CLI_TRY_TEXT(reserved,
                       ...
                       "Getting market reserved amount for address " +
      args.address
     ...
     the " "default wallet address"); option("amount",
